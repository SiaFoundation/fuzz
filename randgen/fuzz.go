	...
	acc := f.accs[addr]
	{
		...
		for i, m := 0, f.rng.Intn(maxOutputs); i < m; i++ {
			...
			if f.prob(probSC) && balanceSC.Cmp(rSC) >= 0 {
				...
				txn.SiacoinOutputs = append(txn.SiacoinOutputs, types.SiacoinOutput{Value: rSC, Address: f.randAddr(addr)})
			}
			rSF := f.randSF()
			if f.prob(probSF) && balanceSF >= rSF {
				balanceSF = balanceSF - rSF
				txn.SiafundOutputs = append(txn.SiafundOutputs, types.SiafundOutput{Value: rSF, Address: f.randAddr(addr)})
			}
		}
	}
	{
